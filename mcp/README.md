# Hướng dẫn Deploy HerFit MCP Server (Supabase + SSE)

Tài liệu này hướng dẫn cách deploy và chạy dịch vụ HerFit MCP Server sử dụng Supabase và SSE Transport trên VPS Ubuntu.
...
2. Dán nội dung cấu hình mẫu sau đây (hãy điều chỉnh lại `User` và `WorkingDirectory` cho chính xác nếu cần):
   ```ini
   [Unit]
   Description=HerFit MCP Server (Supabase + SSE)
